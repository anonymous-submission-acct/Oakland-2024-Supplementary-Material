# https://github.com/boppreh/objregex
...
LeafPatternType = Union[Item, Callable[[Match[Item]],
                                       Union[bool, int, Iterator[Match[Item]]]]]
...
def _match_sequence(pattern: Sequence[LeafPatternType[Item]], match: Match[Item]) -> Iterator[Match[Item]]:
    ...
    for subpattern in pattern:
        matches = [next_match for match in matches for next_match in _next_match(
            subpattern, match)]
        if not matches:
            return
    yield from matches
...
def repeat(pattern: PatternType[Item], min_n: int = 1, max_n: Optional[int] = None) -> PatternType[Item]:
    ...
    def wrapper(match: Match[Item]) -> Iterator[Match]:
        ...
        for n in range(len(match.items)):
            matches = [
                next_match for match in matches for next_match in _next_match(pattern, match)]
...
def scan(patterns: Dict[str, PatternType[Item]], items: Sequence[Item]) -> Iterator[Tuple[str, Match[Item]]]:
    ...
    while match.end < len(items):
        matches = [(name, new_match) for name, pattern in patterns.items()
                   for new_match in _next_match(pattern, match)]
        if not matches:
            return
